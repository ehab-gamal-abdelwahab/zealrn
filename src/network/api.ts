...
export const deleteUser = ({token, email}: any) =>
  axios
    .delete(`${DOMAIN}${apiTokens.users}/${email}`, {headers: {token: token}})
    .then(response => response.data)
    .catch(error => Promise.reject(error));

export const userLocations = ({token, email}: any) =>
  axios
    .get(`${DOMAIN}${apiTokens.locations}/${email}`, {headers: {token: token}})
    .then(response => response.data)
    .catch(error => Promise.reject(error));

export const deleteLocation = ({token, id}: any) =>
  axios
    .delete(`${DOMAIN}${apiTokens.locations}/${id}`, {headers: {token: token}})
    .then(response => response.data)
    .catch(error => Promise.reject(error));
